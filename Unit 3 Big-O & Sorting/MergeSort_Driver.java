...
class MergeSort
{
   ...
   */
   ...
   {
      System.arraycopy(array, low, copyBuffer, low, high-low+1);
      int i1 = low;
      int i2 = middle+1;
 
      for (int i = low; i <= high; i++) {
         if (i1 > middle)
            array[i] = copyBuffer[i2++];
         else if (i2 > high)
            array[i] = copyBuffer[i1++];
         else if (copyBuffer[i1] <= copyBuffer[i2]) 
            array[i] = copyBuffer[i1++];
         else
            array[i] = copyBuffer[i2++];
      }
                                   ...
                                   int low, int middle, int high)
   {
     ...
     for (int i = low; i <= high; i++) {
      if (i1 > middle)
         ...
         array[i] = copyBuffer[i1++];
      else if (copyBuffer[i1].compareTo(copyBuffer[i2]) <= 0)
         array[i] = copyBuffer[i1++];
      else
      array[i] = copyBuffer[i2++];
     }
